...
/**
 * A sharded `Tracker` has a set of recorded datapoints
 * For each track common cumulative computations can be run:
 * aggregate, averages, high/low, topK (e.g. the top N highest temperatures).
 *
 * Note that since this tracker is not storing its state anywhere else than in JVM memory, if Akka Cluster Sharding
 ...
 */
final class Tracker extends AbstractBehavior<Tracker.Command> {
  
  // setup for using Trackers through Akka Cluster Sharding
  // these could also live elsewhere and the Tracker class be completely
  // oblivious to being used in sharding
  public static final EntityTypeKey<Command> TypeKey =
    EntityTypeKey.create(Tracker.Command.class, "Tracker");

  public static void initSharding(ActorSystem<?> system) {
    ClusterSharding.get(system).init(Entity.of(TypeKey, entityContext ->
      Tracker.create(entityContext.getEntityId())
    ));
  ...
  public static final class QueryResult implements CborSerializable {
    public final String wsid;
    public final Tracker.DataType dataType;
    public final Tracker.Function function;
    public final int readings;
    public final List<TimeWindow> value;
    @JsonCreator
    public QueryResult(String wsid, Tracker.DataType dataType, Tracker.Function function, int readings, List<TimeWindow> value) {
      this.wsid = wsid;
      this.dataType = dataType;
  ...
  // small domain model for querying and storing tracker data
  ...
  public Tracker(ActorContext<Command> context, String wsid) {
    super(context);
    this.wsid = wsid;

    System.out.println("Created the Tracker: " + wsid);
  }
  ...
  private Behavior<Command> postStop() {
    getContext().getLog().info("Stopping, losing all recorded state for tracker {}", wsid);
    return this;
  }
