...
public class TrackSensor extends AbstractBehavior<TrackSensor.Command> {
  ...
  public static Behavior<Command> create(String wsid, SimulatorSettings settings, int httpPort) {
    return Behaviors.setup(context ->
        Behaviors.withTimers(timers ->
          new TrackSensor(context, timers, wsid, settings, httpPort)
        )
    );
  }
  ...
  // TODO: TrackSensor -> MockTrackSensor?
  private TrackSensor(ActorContext<Command> context, TimerScheduler<Command> timers, String wsid, SimulatorSettings settings, int httpPort) {
    super(context);
    this.wsid = wsid;
    this.settings = settings;
    this.timers = timers;
    this.http = Http.get(Adapter.toClassic(context.getSystem()));
    sensorUrl = "http://" + settings.host + ":" + httpPort + "/tracker/" + wsid;
    objectMapper = JacksonObjectMapperProvider.get(getContext().getSystem()).getOrCreate("track-simulator", Optional.empty());
  ...
  private Behavior<Command> onSample() throws Exception {
    ...
    getContext().getLog().debug("Recording track measurement {}", value);
  ...
  private void recordTemperature(long eventTime, double value) throws Exception {
    ...
    CompletionStage<String> futureResponseBody =
        http.singleRequest(HttpRequest.POST(sensorUrl).withEntity(ContentTypes.APPLICATION_JSON, json))
